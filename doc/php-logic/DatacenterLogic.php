<?php

namespace Business\Config;
use \Tools\Tools;
use \Mysql\Sw\DatacenterModel as Datacenter;

/**
 *
 */
class DatacenterLogicModel extends \Business\Config\AbstractModel {

    //验证的规则和字段
    public static $validate_list = array(
    );

    //不能为空的字段
    public static $not_empty_fields = array(
        'name',
        'short_tag',
        'area',
        'pro_city',
        'detail_addr',
        'charge',
        'net_charge',
        'acc_man',
        'acc_man_phone',
        'duty_phone'
    );

    //csv表头
    public static $fields_array = array(
        'id' => 'ID',
        'name'=>'机房名称',
        'short_tag'=>'机房缩写',
        'area'=>'区域名称',
        'pro_city'=>'省市',
        'detail_addr'=>'详细地址',
        'charge'=>'机房负责人',
        'net_charge'=>'网络负责人',
        'acc_man'=>'大客户经理',
        'acc_man_phone'=>'大客户经理电话',
        'duty_phone'=>'值班电话',
        'remark'=>'备注'
    );

    //搜索的条件
    public static $search_fields = array(
        'name'=>'机房名称',
        'short_tag'=>'机房缩写',
        'area'=>'区域名称'
    );

    //可修改的字段
    public static $update_fields = array(
        'name'=>'机房名称',
        'short_tag'=>'机房缩写',
        'area'=>'区域名称',
        'pro_city'=>'省市',
        'detail_addr'=>'详细地址',
        'charge'=>'机房负责人',
        'net_charge'=>'网络负责人',
        'acc_man'=>'大客户经理',
        'acc_man_phone'=>'大客户经理电话',
        'duty_phone'=>'值班电话',
        'remark'=>'备注'
    );



    public function getList($query = array())
    {
        $query = array_filter($query);
        $res = array();
        $data = Datacenter::getInstance()->getList($query);
        $res = $data;
        return $res;
    }

    /**
     * 获取单条
     * @param $query array
     * @return array
     */
    public function getInfo($query = array()) {
        $query = array_filter($query);
        $res = array();
        $data = Datacenter::getInstance()->getInfo($query);
        if(!empty($data)) {
            $res = $data;
        }
        return $res;
    }

    /**
     * 添加单条
     * @param $data array
     * @param $return_type 'last_insert_id' 'last_insert_data'
     * @return array
     */
    public function addInfo($data, $return_type = null) {
        $res = array();
        $res =  Datacenter::getInstance()->addInfo($data, $return_type);
        return $res;
    }

    /**
     * 添加多条
     * @param $data array
     * @param $return_type 'last_insert_id' 'last_insert_data'
     * @return array
     */
    public function addList($data, $return_type = null) {
        $res = array();
        $res =  Datacenter::getInstance()->addList($data, $return_type);
        return $res;
    }

    /**
     * 更新映射信息
     * @param $set array 更新内容
     * @param $where array 条件
     * @return array
     */
    public function updateInfo($set, $where, $return_type = null) {
        $res = array();
        $res =  Datacenter::getInstance()->updateInfo($set, $where, $return_type);
        return $res;
    }

    /**
     * 删除
     * @param $where array
     * @return array
     */
    public function deleteInfo($where) {
        $res =  Datacenter::getInstance()->deleteInfo($where);
        return $res;
    }


    /**
     * 销毁类实例
     */
    function __destruct()
    {
    }

    /**
     * 单例模式
     * @var \Mysql\Sw\DatacenterModel
     */
    private static $_instance = null;

    public static function getInstance() {
        if (!(self::$_instance instanceof self)) {
            self::$_instance = new self();
        }
        return self::$_instance;
    }
}
